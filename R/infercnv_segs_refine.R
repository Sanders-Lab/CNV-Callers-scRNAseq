...
tnbc_cells <- tnbc_cells %>% dplyr::rename(
    seqnames = "chr",
    end = "stop"
)
...
tnbc_cells_label[, 1:5][tnbc1_AAACCTGCACCTTGTC != ""]
...
system.time(
    infercnv_cnv_segs <- foreach(
        cell_num = 5:ncol(tnbc_cells_label),
        .combine = "rbind"
    ) %dopar% {
        if (tnbc_cells_label[, cell_num] %>% grep("amp", .) %>% length() > 0 |
            tnbc_cells_label[, cell_num] %>%
                grep("del", .) %>%
                length() > 0) {
...
tnbc_cells_label <- as.data.table(tnbc_cells_label)
tnbc_cells_label
tnbc_cells_label[seqnames == "chr20"][, 1:5] |>
    print(n = 194)
...
dig_kar <- plot_digital_karyotype(
    plot_ideo_only = F,
