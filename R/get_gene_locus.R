...
hgnc_order <- getBM(
    attributes = c("hgnc_id", "hgnc_symbol", "chromosome_name", "start_position", "end_position"),
    filter = "hgnc_id",
    values = hgnc_id,
    mart = h_mart,
    useCache = F
...
infercnv_gene_range$chromosome_name %>%
    is.na() %>%
    table()
infercnv_gene_range$start_position %>%
    is.na() %>%
    table()
infercnv_gene_range$end_position %>%
    is.na() %>%
    table()
...
gene_order <- getBM(
    attributes = c("hgnc_id", "chromosome_name", "start_position", "end_position"),
    filter = "hgnc_id",
    values = gene_list,
    mart = h_mart,
    useCache = F
...
exon_names <- getBM(
    attributes = c("hgnc_symbol", "ensembl_exon_id"),
    filter = "hgnc_symbol",
    values = gene_list,
    mart = h_mart,
    useCache = F
...
gene_order <- getBM(
    attributes = c("uniprot_gn_symbol", "chromosome_name", "start_position", "end_position"),
...
transcript_order <- getBM(
    attributes = c("external_transcript_name", "chromosome_name", "start_position", "end_position"),
