...
r <- tnbc_cells[, 1:4] %>%
    group_by(seqnames) %>%
    group_map(~ rle(.$"tnbc1_AAACCTGCACCTTGTC"))
...
SegsChromCell <- function(cell_chr_df) {
    rle_chr_cell <- cell_chr_df %>%
        pull(4) %>%
        rle()


    tmp_df <- data.frame(
        seqnames = character(length(rle_chr_cell$lengths)),
